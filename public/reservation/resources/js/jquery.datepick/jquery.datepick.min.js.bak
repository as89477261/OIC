	...
	var e={
		...
		getConfig:function(){
			return{
				...
				yearTextOffset:this.get('yearTextOffset')
				};
...
_updateInput:function(a,b){
	...
	if(c){
		...
		if(!c.inline&&!b){
			...
			var data = e.split("-");
			console.log(data);
			data[2] = parseInt(data[2])+z;
			dd = data[0]+"-"+data[1]+"-"+data[2];
			$(a).val(d) //This is default
			//$(a).val(dd);
		}
...
_generateMonthSelection:function(a,b,c,d,e,f){
	...
	i+='</select>';
h=h.replace(/\\x2E/,i);
var offsetYearText = function(y, inst){return y + inst.get('yearTextOffset');}
var j=a.get('yearRange');
if(j=='any'){
	i='<select class="'+this._monthYearClass+' '+this._anyYearClass+'" title="'+a.get('yearStatus')+'">'+'<option>'+offsetYearText(b, a)+'</option></select>'+'<input class="'+this._monthYearClass+' '+this._curMonthClass+c+'" value="'+b+'">';
}else{
	j=j.split(':');
	...
	var o=function(y){
		if(y!=0){
			i+='<option value="'+c+'/'+y+'"'+(b==y?' selected="selected"':'')+'>'+offsetYearText(y, a)+'</option>';
		}
	};
