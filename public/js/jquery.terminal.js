	...
	$.fn.terminal = function(url, options) {
		...
		return this
				.each( function() {
					...
					terminal.append('<form method="' + settings.form_method
							+ '" action="' + url + '"></form>');
					var terminal_form = $(this).find('form:last');

					if (settings.custom_prompt) {
						terminal_form
								.append('<span id=\'firstPrompt\'>' + settings.custom_prompt + '</span>');
					}

					terminal_form.css('display', 'inline');
					/*terminal_form.attr('onsubmit',
					...
					*/
					...
					var terminal_append = function(data) {
						...
						if(data == 'login successfully\n') {
							//alert('xxx');
							$('#firstPrompt').html('<font color="blue">[ECM]</font>> ');
							settings.custom_prompt = '<font color="blue">[ECM]</font>> ';
						}
						if(data == 'logout successfully\n')  {
							$('#firstPrompt').html('<font color="red">[$]</font>> ');
							settings.custom_prompt = '<font color="red">[$]</font>> ';
						}
						
						var formattedData = $('<span></span>').text(data);
						//alert (formattedData.text());
						var dataRows = formattedData.text().split(/\n/);
						
						data = '';
						
						for (row in dataRows) {
							//alert (dataRows[row]);
							data += '<span>' + dataRows[row].replace(/\t/g,
									tabString);
						...
						// if input is disabled, we trick a focus so the
						// scrolling will be ok
						if (settings.disable_input) {
							terminal_input.removeAttr('disabled').focus().attr(
									'disabled', 'disabled');
						}
					}
					...
					var terminal_command = function(first) {
						...
						/*
						 * $.ajax({ type: 'POST', url: '/{$config
						 * ['appName']}/ECMJS/check-session', dataType: 'json',
						 * success: function(o){ Ext.MessageBox.hide();
						 * if(o.redirectLogin == 1) { sessionExpired(); } else {
					...
					terminal_form
							.append('<input type="text" name="' + settings.input_name + '" />');
							...
							.ready(
									function() {
										...
										if (settings.hello_message)
											terminal_output
													.append('<span>' + settings.hello_message + '<br /></span>');
