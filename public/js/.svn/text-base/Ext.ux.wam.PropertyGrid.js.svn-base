...
Ext.ux.wam.PropertyColumnModel = function(store){
    ...
    this.editors = {
    	'secretEditor': new Ext.grid.GridEditor(
...
Ext.extend(Ext.ux.wam.PropertyColumnModel, Ext.grid.ColumnModel, {
    ...
    renderCell : function(value,metadata,record){
        var rv = value;
        if(record.data['editor'] == 'secretEditor') {
        	return secretRenderer(value);
        }
    ...
    getCellEditor : function(colIndex, rowIndex){
        ...
        if(p.data['editor']!==""){
        	if(p.data['editor']=="secretEditor") {
        		return this.editors['secretEditor'];
        	} else {
