//+------------------------------------------------------------------+
//|                                       JumStoCh_Improved_v3.mq4    |
//|                 Improved Version with Low DD & Hedge Closing      |
//|                 Based on Jum+StoCh v2.5 + Low DD Improvements     |
//+------------------------------------------------------------------+
#property copyright "Improved by Claude AI"
#property link      ""
...
input double Target_Persen = 5.0;             // Target Profit %

input string Section2 = "=== Hedge Pair Closing System ===";
input bool   Enable_Hedge_Closing = TRUE;     // Enable Hedge Pair Closing
input int    Min_Orders_For_Hedge = 3;        // Min Orders to Enable Hedge Close
input double Hedge_Profit_Target = 1.0;       // Hedge Close Profit % (of balance)
input bool   Hedge_In_Money = FALSE;          // Use Money Instead of %
input double Hedge_Profit_Money = 10.0;       // Hedge Close Profit (Money)
...
input int    Magic = 69;                      // Magic Number

input string Section4 = "=== Grid Settings (DD Reduced) ===";
input bool   Use_Dynamic_Range = TRUE;        // Use ATR for Dynamic Range
input double Range = 30.0;                    // Grid Range (pips) - Increased
input double DiMarti = 1.3;                   // Martingale Multiplier (Reduced!)
input int    Level_Max = 6;                   // Max Grid Levels (Reduced!)
input bool   Use_Stop_Loss = TRUE;            // Enable/Disable Stop Loss
input double SL = 200.0;                      // Stop Loss (pips) - Reduced
input bool   Use_Take_Profit = TRUE;          // Enable/Disable Take Profit
input double TP = 25.0;                       // Take Profit (pips)

input string Section5 = "=== Breakeven & Trailing ===";
input int    Star_ModifTp_Bep = 3;            // Start Modify TP from Level
input double Tp_from_Bep = 8.0;               // TP from Breakeven
input bool   Dtrailing = TRUE;                // Enable Trailing Stop
input int    StartTrail = 15;                 // Start Trailing (pips)
input int    Trailing = 10;                   // Trailing Distance (pips)
...
input string Section8 = "=== Indicators (Improved Confirmation) ===";
input int    kperiod = 32;                    // Stoch K Period
input int    dperiod = 12;                    // Stoch D Period
input int    slowing = 12;                    // Stoch Slowing
input int    lo_level = 25;                   // Stoch Low Level
input int    up_level = 75;                   // Stoch High Level
input int    maPereode = 25;                  // MA Period

input string Section9 = "=== Additional Filters ===";
input int    RSI_Period = 14;                 // RSI Period
input double RSI_Oversold = 30;               // RSI Oversold
input double RSI_Overbought = 70;             // RSI Overbought
input int    ATR_Period = 14;                 // ATR Period
input double ATR_Volatility_Filter = 2.5;     // Max ATR Multiplier
...
int OnInit()
{
   ...
   Print("===== Improved Jum+StoCh EA v3.0 Initialized =====");
