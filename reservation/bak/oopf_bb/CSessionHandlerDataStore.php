...
class CSessionHandlerDataStore extends CDataStore
{
	var $lifetime;
	...
	{
		list($sess_info) = $this->dataStore->getData(array('SessionId' => $sid));
	...
	function read($sid)
	{
		...
		$sess_info = $this->dataStore->getData(array('SessionId' => $sid, 'IPAddress' => $ip, 'EndedTime' => null));	   // *** check ended time by itself rather than by db
	...
	function write($sid, $data)
	{
		...
		error_log(serialize($sess_info)); error_log(serialize($where)); error_log($data);
		// lifetime cannot be changed after cookie is created
		if ($this->dataStore->result_count > 0)
			$this->dataStore->update(array('Information' => $data, 'LastChanged' => $current_time), $where);
		else
			$this->dataStore->insert(array('SessionId' => $sid, 'IPAddress' => $ip, 'InsertedTime' => $current_time, 'LastChanged' => $current_time, 'Lifetime' => $this->lifetime, 'Information' => $data), $dummy);
		return true;
	}
	...
	{
		$current_time = getCurrentDateTime();
		$sess_info = $this->dataStore->getData(array('SessionId' => $sid));
		if ($this->result_count > 0)
			return $this->dataStore->update(array('EndedTime' => $current_time), array('SessionId' => $sid));
		else
			return false;
	}
