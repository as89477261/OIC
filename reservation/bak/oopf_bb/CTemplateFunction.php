...
{
	var $mag_symbol = array('', 'k', 'M', 'G', 'T', 'P', 'E', 'Z');			// skip 'Y' because we need it to enable the feature.
	var $mag_order;

	function init()
	{
		$this->mag_order = array_flip($this->mag_symbol);
	}

	function switchParam($p, $default = ',')
	{
		switch($p) {
			case '':			// if nothing specified, then it should be ','
			case CONST_YES:  return $default;
			case CONST_NO: return '';
			default: return $p;
		}
	}

	function calculateOrderMag($val, $base = CONST_YES)
	{
		if (CONST_YES == $base)
			$e = floor(log10($val) / 3);
		else
		{
			if (! is_numeric($base))
			{
				if (array_key_exists($base, $this->mag_order))
					$e = $this->mag_order[$base];
				else
					$e = floor(log10($val) / 3);			// if not found, go to default
			}
			elseif ($base > 21)			// if too big order, go to default
				$e = floor(log10($val) / 3);
			else
				$e = $base;
		}
		...
		$val = $val / (pow(1000, $e));
		return array($val, $this->mag_symbol[$e]);
	}
    ...
    function evaluate(&$data, $name, $param)
    {
        ...
        else
        {
            ...
            if ($cnt > 3)
            {
	            list($val, $mag_symbol) = $this->calculateOrderMag($val, $p[3]);
	            if ($cnt > 4)
	            {
	            	switch ($p[4] == CONST_YES)
	            	{
		            	case '':
		            	case CONST_NO: $mag_symbol = ''; break;
		            	default: $mag_symbol = $p[4]; break;
	            	}
            	}
            }
			else
    	...
    	return number_format($val, $decimal, $period, $thousand) . $mag_symbol;
    }
}
