		...
		foreach ($to_join as $tablename => $fieldlist)
		{
			...
			if (is_array($fieldlist))
			{
				foreach ($fieldlist as $source => $destination)
				{
					...
					{
// the following line is needed to rewrite to get datastore from parent because CDataStore is now just abstract class
	...
	function generateCsvList($arg_fields = array(), $add_table_name = false, $convert_date = false)
	{
		...
		elseif (is_scalar($arg_fields))
		{
			...
			if ($arg_fields == '*')
			{
				$sql_date_convert = $this->generateConvertDateColumns($add_table_name);
				$arg_fields .= ($sql_date_conver ? ',' . $sql_date_convert : '');
			}
			return $arg_fields;
		}
		else
...
*/
	...
	function select($arg_fieldlist = '*', $arg_where = array(), $arg_jointable_list = '', $arg_orderby = array(), $arg_groupby = array())
	{
		...
		$sql = 'SELECT ' . $this->generateCsvList($arg_fieldlist, true, true) . ' FROM ' . $this->db_table;
