...
class CDataStoreMySql extends CDataStore
{
	...
	var $count_key = 'Count';
        var $create_sql = 'CREATE TABLE %s ( %s , PRIMARY KEY ( %s ))';
        var $data_type = array(DT_VARCHAR => 'varchar', DT_TEXT => 'text', DT_INTEGER => 'int', DT_DECIMAL => 'decimal', DT_DATE => 'date', DT_DATETIME => 'datetime', DT_BOOLEAN => 'char(1)');
