...
function extractArrayValue(&$arg_input, $arg_key)
{
	...
	if (is_array($arg_input))
		foreach ($arg_input as $row)
			if (is_array($row) && array_key_exists($arg_key, $row))
...
/**
 * คัดลอกข้อมูลที่ต้องการจาก array ด้วย key ของ array
 *
 * @param <array> $arg_input
 * @param <mixed> $arg_keys
 * @param <array> $arg_merge_array
 * @return <array>
 */
...
function copyArrayValueNotBlank(&$arg_input, $arg_keys, $arg_merge_array = array())
{
	if (is_array($arg_merge_array))
		$arg_output = &$arg_merge_array;
	foreach ($arg_keys as $key)
		if (array_key_exists($key, $arg_input) && ($arg_input[$key] !== null) && ($arg_input[$key] !== ''))
			$arg_output[$key] = $arg_input[$key];
	return $arg_output;
}
...
{
	if (! is_array($arg_row)) return '';
	foreach ($arg_keys as $akey)
		$key[] = (array_key_exists($akey, $arg_row) ? $arg_row[$akey] : '');
...
function mergeDescription(&$arg_data_rows, $arg_desc_rows, $arg_keys, $arg_fields = '*', $merge_type = MERGEDESC_TYPE_SIMPLE, $merge_attr = 'Merged')
{
	...
	if ($arg_fields == '*')
	{
		...
		{
			$desc_ind = getCompoundKey($row, $data_keys);
			$desc = (array_key_exists($desc_ind, $desc_rows) ? $desc_rows[$desc_ind] : '');
			switch ($merge_type)
			{
	...
	$arg_distinct = array(), $arg_concat = array())
{
 ...
 function arrayReplaceKeyRecursive($argValue, $argKey)
{
    $return = array();
    foreach ($argValue as $key => $value) {
        if (is_array($value))
        {
            $value = arrayReplaceKeyRecursive($value, $argKey);
        }
        if (array_key_exists($key, $argKey))
        {
            $key = $argKey[$key];
        }
        $return[$key] = $value;
