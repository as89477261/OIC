...
class CTemplateMacroSelect extends CTemplateMacroForeach
{
    function compile(&$content, &$params, &$input)
    {
        ...
        {
            if (array_key_exists($key_name, $row) && ($compare == $row[$key_name]))
...
/*
<?php BEGIN(CASE,val_name) ?>
<?php CASE(val1,val2,val3,...) ?> valn can be any string, integer, float value, '%NULL' for null, blank for '', or in format '%n' for the case of result of (value % num_of_cases) == n
<?php CASE(%DEFAULT) > default case cannot combine with any other cases
*/
