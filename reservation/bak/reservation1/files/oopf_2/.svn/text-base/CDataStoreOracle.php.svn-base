...
class CDataStoreOracle extends CDataStore
{
	...
	var $date_column_list = false;			// distinguish date columns to add convert function to them in SQL
	var $count_key = 'COUNT';
        var $create_sql = 'CREATE TABLE %s ( %s ) CONSTRAINT PK_%1$s PRIMARY KEY ( %s )';
        var $data_type = array(DT_VARCHAR => 'varchar2', DT_TEXT => 'clob', DT_INTEGER => 'int',
                                        DT_DECIMAL => 'decimal', DT_DATE => 'date', DT_DATETIME => 'date',
                                        DT_BOOLEAN => 'char(1)');
...
//                                                'primary_key'=>'attr_primary_key');
	...
	function generateConvertDateColumns($add_table_name = false)
	{
		$cols = array();
		if ($this->date_column_list)
			foreach ($this->date_column_list as $column)
				$cols[] = 'TO_CHAR(' . ($add_table_name ? $this->db_table . '.' : '') . $column . ",'YYYY-MM-DD') as " . $column;
		if ($this->datetime_column_list)
			foreach ($this->datetime_column_list as $column)
				$cols[] = 'TO_CHAR(' . ($add_table_name ? $this->db_table . '.' : '') . $column . ",'YYYY-MM-DD HH24:MI:SS') as " . $column;
		return implode(',', $cols);
	}
