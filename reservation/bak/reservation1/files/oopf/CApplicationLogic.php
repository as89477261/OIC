...
class CApplicationLogic
{
    ...
    function &createSession($cache_limiter = 'nocache')
    {
    	...
    	// *** need to fix CSessionHandlerDataStore
        $session->session_handler = new CSessionHandlerDataStore($this->db_link, $this->app_info['Session']['SessionDbName'], $this->app_info['Session']['SessionTableName']);
        $session->session_handler->setEncoding($this->app_info['MultiLanguage']['DbEncoding']);
    ...
    function getDbLangaugeSupport(&$ds, $lang_id = '')
    {
        ...
        if (! in_array($ds->db_table, $this->app_info['MultiLanguage']['Tables']))
            return ;
        switch ($this->lang_by)
        {
            case 'db':
