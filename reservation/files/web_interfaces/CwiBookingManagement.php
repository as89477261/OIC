...
class CwiBookingManagement extends CWebInterface
{
	...
	var $function_list = array(
		...
		'check-cheif-id' => array(FN_ID => 'check-cheif-id', FN_NAME => 'checkCheifId', FN_DESC => 'My Booking List')
	);
	...
	function cropTime($booking, $currentDate)
	{
		if (!is_array($booking))
			return $booking;
		...
		{
			if ($book['go_date'] < $currentDate)
			{
				$book['go_hour'] = 0;
				$book['go_minute'] = '01';
			}

			if ($book['back_date'] > $currentDate)
			{
				$book['back_hour'] = 24;
				$book['back_minute'] = 0;
			}
	...
	function margeFormBooking(&$arg_output, &$arg_template)
	{
		...
		$m_booking->order_by = array('go_date' => ORDERBY_ASC, 'go_hour' => ORDERBY_ASC, 'go_minute' => ORDERBY_ASC);
		...
		$cars = $arg_output['car_rows'];

		$carRow = array();

		foreach ($cars as &$car)
		{
			if (!is_array($booking[$car['car_id']]))
				continue;

			$carBooking = $this->cropTime($booking[$car['car_id']], $date);
			...
			{
				$bookRange = array(
					'start_hour' => $book['go_hour'],
					'start_min' => $book['go_minute'],
					'stop_hour' => $book['back_hour'],
					'stop_min' => $book['back_minute']
				);
				if (count($range) > 0)
				{
					if (($bookRange['start_hour'] > $range['stop_hour'])
							|| (($bookRange['start_hour'] == $range['stop_hour']) && ($bookRange['start_min'] > $range['stop_min'])))
					{
						$carRange[] = $range;
						$range = $bookRange;
						$range['books'][] = $book;
					...
					{
						if ($bookRange['stop_hour'] > $range['stop_hour'])
						{
							$range['stop_hour'] = $bookRange['stop_hour'];
							$range['stop_min'] = $bookRange['stop_min'];
						}
						$range['books'][] = $book;
					}
				}
			...
			$carRange[] = $range;
			$carRange = $this->setFormatMin($carRange);
			$car['time_rows'] = $carRange;
		}

		$arg_output['car_rows'] = $cars;

//		ก่อนทำการแก้ไขเป็นแบบใหม่
//
//		$m_booking = new MBooking($this);
//		$m_booking->order_by = array('go_date' => ORDERBY_ASC, 'go_hour' => ORDERBY_ASC, 'go_minute' => ORDERBY_ASC);
//		$date = $m_booking->convertFormate($arg_output['date']);
...
//$arg_output['HttpHeader'][] = 'Location: http://handicap/ECMDev/reservation/./reservation.php?fid=driver-list';
	...
	function createForm(&$arg_output, &$arg_template, $arg_input)
	{
		...
		$create_date = explode('-', (string) substr($arg_output['create_date'],0,10));
		$arg_output['create_day'] = $create_date[0];
		$arg_output['create_month'] = $arrayMonth[$create_date[1]];
		$arg_output['create_year'] = $create_date[2];
...
//		echo '<pre>';print_r($account);
	...
	function showReport(&$arg_output, &$arg_template, $arg_input)
	{
		...
		switch ($arg_input[FN_FN])
		{
			...
			case 'gen-report'://echo '>>'.$arg_input['choice'].'<<';
				...
				$fname = $arg_output['data_report']['real_path'];
	...
	function checkCheifId(&$arg_output, &$arg_template, $arg_input)
	{
		if ($arg_input['cheif_id'] == '')
			echo 2;
		else
		{
			$m_booking = new MBooking($this);
			if ($m_booking->checkCheifId($arg_input['cheif_id'], $arg_input['chief_name']))
				echo 1;
			else
				echo 0;
		}
