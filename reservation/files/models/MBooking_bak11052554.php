...
class MBooking extends CReservationModel
{
	...
	var $string_column_list = array('status', 'comment_detail', 'deparment_id', 'account_id', 'position_id', 'goto', 'address', 'province', 'objective', 'chief_id', 'go_date', 'back_date', 'create_date');
...
//	var $join_models = array(MCar => array('car_id'), MDriver => array('driver_id'));
	...
	function countbookingByDriver0($arg_input, $drivers, $driverBookings)
	{
		...
		{
			$worksheet->set_column($colWidth, $colWidth, 10);
			$worksheet->set_column($colWidth + 1, $colWidth + 1, 10);
			$colWidth += 2;
		}
		...
		$worksheet->write(chr(65) . $count_row_xls, 'ชื่อ', $format2);
		$worksheet->write(chr(66) . $count_row_xls, '', $format2);

		$worksheet->merge_cells(chr(65) . $count_row_xls . ':' . chr(66) . $count_row_xls);

		$worksheet->write(chr(65) . ($count_row_xls + 1), 'ปี', $format2);
		$worksheet->write(chr(66) . ($count_row_xls + 1), 'เดือน', $format2);
		$count_col_xls_merge = $count_col_xls;
		foreach ($drivers as $value)
		{
			$worksheet->write(chr($count_col_xls_merge) . $count_row_xls, $value['first_name'] . ' ' . $value['last_name'], $format2);
			$worksheet->write(chr($count_col_xls_merge + 1) . $count_row_xls, '', $format2);

			$worksheet->write(chr($count_col_xls_merge) . ($count_row_xls + 1), 'ส่วนกลาง', $format2);
			$worksheet->write(chr($count_col_xls_merge + 1) . ($count_row_xls + 1), 'ต่างจังหวัด', $format2);

			$worksheet->merge_cells(chr($count_col_xls_merge) . $count_row_xls . ':' . chr($count_col_xls_merge + 1) . $count_row_xls);
		...
		for ($cYear = $srYear; $cYear <= $spYear; $cYear++)
		{
			...
			$count_col_xls = 65;
			$worksheet->write(chr($count_col_xls) . $count_row_xls, $cYear, $format4);
			$blank = 0;
			for ($cMounth = $srMount; $cMounth <= $spMount; $cMounth++)
			{
				...
				if ($cMounth < 10 && strlen($cMounth) < 2)
					$mounth = '0' . $mounth;
				$count_col_xls = 66;
				$worksheet->write(chr($count_col_xls) . $count_row_xls, $this->month[$mounth], $format4);
				if ($blank != 0)
					$worksheet->write(chr(65) . $count_row_xls, '', $format4);
				$blank = 1;
				$count_col_xls++;
...
//echo $driver['driver_id'];
					...
					$worksheet->write(chr($count_col_xls) . $count_row_xls, $countIn, $format3);
					$worksheet->write(chr($count_col_xls + 1) . $count_row_xls, $countOut, $format3);
					$count_col_xls += 2;
				}
		...
		$worksheet->write(chr(65) . $count_row_xls, 'รวม', $format5);
		$worksheet->write(chr(66) . $count_row_xls, '', $format3);
		$worksheet->merge_cells(chr(65) . $count_row_xls . ':' . chr(66) . $count_row_xls);
		...
		foreach ($drivers as $value)
		{
			...
			$worksheet->write(chr($count_col_xls) . $count_row_xls, $countIn, $format5);
			$worksheet->write(chr($count_col_xls + 1) . $count_row_xls, $countOut, $format5);
	...
	function countbookingByDriver1($arg_input, $drivers, $driverBookings)
	{
		...
		$worksheet->write(chr(66) . $count_row_xls, 'ชื่อ', $format2);
		$worksheet->write(chr(66) . ($count_row_xls + 1), 'ปี', $format2);
		$count_col_xls_merge = $count_col_xls;
		foreach ($drivers as $value)
		{
			$worksheet->write(chr($count_col_xls_merge) . $count_row_xls, $value['first_name'] . ' ' . $value['last_name'], $format2);
		...
		$worksheet->write(chr(66) . $count_row_xls, 'รวม', $format5);
