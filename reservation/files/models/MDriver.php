...
class MDriver extends CReservationModel
{
	...
	{
		return $this->updateById($driverId, array('status' => $status));
	}

	function setTypeDriver($arg_output)
	{
		$type = makeAssocArray($this->getDriverType(), 'Value');
		foreach($arg_output as &$value)
		{
			$value['type'] = $type[$value['type']]['Text'];
		}
		return $arg_output;
	}
