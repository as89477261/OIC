...
class MBooking extends CReservationModel
{
	...
	var $string_column_list = array('status', 'comment_detail', 'deparment_id', 'account_id', 'position_id', 'goto', 'address', 'province', 'objective', 'chief_id', 'go_date', 'back_date', 'create_date', 'road', 'zone');
...
//		$ds->debug_sql = true;
	...
	function countbookingByDriver0($arg_input, $drivers, $driverBookings)
	{
		...
		for ($cYear = $srYear; $cYear <= $spYear; $cYear++)
		{
			...
			$count_col_xls = 65;
			$worksheet->write(chr($count_col_xls) . $count_row_xls, 'ปี ' . $cYear, $format4);
			$blank = 0;
...
//echo $driver['driver_id'];
		...
		$worksheet->write(chr(65) . $count_row_xls, 'รวม', $format3);
	...
	function countbookingByDriver1($arg_input, $drivers, $driverBookings)
	{
		...
		$worksheet->write(chr(66) . $count_row_xls, 'รวม', $format3);
