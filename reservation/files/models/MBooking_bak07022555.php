...
//		$ds->debug_sql = true;
	...
	function countbookingByDriver0($arg_input, $drivers, $driverBookings)
	{
		...
		foreach ($drivers as $value)
		{
			...
			$worksheet->write($this->charector[$count_col_xls_merge] . ($count_row_xls + 1), '???????', $format2);
			$worksheet->write($this->charector[$count_col_xls_merge + 1] . ($count_row_xls + 1), '?????????', $format2);
