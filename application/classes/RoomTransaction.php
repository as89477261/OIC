...
class RoomTransaction
{
    ...
    public static function getBookingTitle($transaction)
    {
        ...
        $title .= '<br />เรื่องประชุม : <b>' . $transaction['F_SUBJECT'] . '</b>';
        $title .= '<br />ประธาน : ' . $transaction['F_CHAIRMAN'];
        $title .= '<br />ผู้เข้าร่วมประชุมภายใน : ' . $transaction['F_QUANTITY_IN'];
        $title .= '<br />ผู้เข้าร่วมประชุมภายนอก : ' . $transaction['F_QUANTITY_OUT'];
        list($y, $m, $d) = explode('-', $transaction['F_CREATETIME']);
        $title .= '<br />วันที่จอง : ' . $d . '/' . $m . '/' . ($y + 543) . ' ' . $transaction['F_CREATE_TIME'];
        $title .= '<br />ผู้จอง : ' . $transaction['F_NAME'] . ' ' . $transaction['F_LAST_NAME'];
        $title .= '<br />หมายเหตุ : ' . $transaction['F_REMARK'];
