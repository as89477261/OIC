...
class ReceivedItemPortlet {
	...
	public function getUI() {
		...
		$genBookSubMenu = "{
				...
				hidden: {$hideSendExternalCommand}
			},/*{ //   คุณลัยแจ้งให้ปิดการใช้งานเมื่อ 29-02-2012
        ...
        function genInternalCircBookno_{$programID} (btn) {
             if(btn == 'yes') { 
                ...
                saveECMData('refDocID',{$gridName}.getSelectionModel().getSelected().get('docID'));
                saveECMData('refTitle',{$gridName}.getSelectionModel().getSelected().get('title'));
				saveECMData('refFrom',{$gridName}.getSelectionModel().getSelected().get('from'));
                ...
                sendInternalForm.getForm().setValues([
					...
					{id:'sendIntSendFrom',value:  getECMData('refFrom')},
            ...
            if(btn == 'yes') {
                if(btn == 'yes') {
                    ...
                    sendInternalGlobalForm.getForm().setValues([
						...
						{id:'sendIntGlobalSendFrom',value:  getECMData('refFrom')}
                    ]);                                                                                                          
                }               
            }
        }
		...
		function genExtBookGlobal_{$programID}(btn) { 
			if(btn == 'yes') {
                ...
                sendExternalGlobalForm.getForm().setValues([
                    ...
                    {id:'sendExtGlobalTitle',value:  getECMData('refTitle')},
                    {id:'sendExtGlobalRefBookno',value:  getECMData('refBookNo')},
					{id:'sendExtGlobalSendFrom',value:  getECMData('refFrom')},					
                    {id:'sendExtGlobalDocNo',value:  getECMData('refOrgDocCode')+'/'}                    
                ]);
	    ...
	    var cm_{$programID} = new Ext.grid.ColumnModel([smc,{
		...
		{$gridName}.on('rowdblclick',function() {
			if ({$gridName}.getSelectionModel().hasSelection()) {
			Cookies.set('docRefID',{$gridName}.getSelectionModel().getSelected().get('recvID'));	//viewDocumentCrossModule('viewDOC_'+{$gridName}.getSelectionModel().getSelected().get('recvID'),{$gridName}.getSelectionModel().getSelected().get('title'),'RI',{$gridName}.getSelectionModel().getSelected().get('docID'));
			{$dblClickFunction}
	    	//alert('dbl click');
		}
		}
	...
	{$gridName}.on({
			'rowclick' : function() {
				var arr = gridReceivedItem.getSelectionModel().getSelections();
				var firsttime = true;
				var cmpvalue = 0;
				...
				Ext.each(arr, function(r, index){
					if (firsttime) {
						cmpvalue = r.get('recvType');
						firsttime = false;
					}
					if(r.get('recvType') != cmpvalue){
						Ext.MessageBox.show({
							title: 'แจ้งให้ทราบ',
							msg: 'เลือกเฉพาะหนังสือประเภทเดียวกันเท่านั้น',
							buttons: Ext.MessageBox.OK,
							icon: Ext.MessageBox.ERROR
						});
						gridReceivedItem.getSelectionModel().deselectRow(gridReceivedItem.getStore().indexOf(r));
						counter--;
					}
					else
						if(con){
							con = con + ',' + r.get('recvID');
						}else{
							con = r.get('recvID');
						}
						Cookies.set('fwD', con);
						//Cookies.set('recvRecordID' + index, r.get('recvID'));
				});
                ...
                */
				...
				Cookies.set('recvRecordCount', counter);
			},
			scope: this
		});
