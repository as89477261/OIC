...
class OrderAssignedPortlet {
	...
	public function getUI() {
		...
		$DIVName = "OrderAssignedUIDIV";
		$programID = "OA_0";
		$gridName = 'gridOrderAssignedItem';
		$filterMenuName = 'OAFilteredMenu';
	    ...
	    $regBookID = 0;
	    $storeName = "OAStore_{$regBookID}";
	    $RIStore = $dfStore->getOrderAssignedStore($regBookID,$storeName);
	    ...
	    var cm_{$programID} = new Ext.grid.ColumnModel([{
               ...
               header: \"{$lang['common']['orderReceiver']}\",
               dataIndex: 'receiver',
               width: 150
            },{
		...
		tb_{$programID}.add({
            ...
            handler: function() {
            	...
            	Ext.getCmp('filterRecvType').setValue('OrderAssigned');
			}
        },{
        	...
        	id: 'btnReAssign_{$programID}',
            text:'แก้ไขการ{$lang['workitem']['assign']}',
            iconCls: 'assignIcon',
            disabled: true,
            handler: function(e) {
                Ext.MessageBox.show({
                ...
                Ext.Ajax.request({
                    ...
                    success: function(o){
                        ...
                        if(r.redirectLogin == 1) {
                            ...
                            reAssignOrderWindow.show();
                            reAssignOrderForm.getForm().reset();
                            reAssignOrderForm.getForm().setValues(
                            [
                                {id: 'orderRecieverRefID2',value: {$gridName}.getSelectionModel().getSelected().get('recvID')},
                                {id: 'orderID',value: {$gridName}.getSelectionModel().getSelected().get('orderID')}
                            ]
                            );
                        }
                    }
            ...
            handler: function(){
                ...
                Ext.getCmp('btnReAssign_{$programID}').disable();    
	...
	{$gridName}.on({
			'rowclick' : function() {
                ...
                Ext.getCmp('btnReAssign_{$programID}').enable();      
