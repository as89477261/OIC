...
class Fenrir_Utility_AutoLoader implements Zend_Loader_Autoloader_Interface {
	public static function loadClass($class, $dirs = null) {
		global $config;
		
		if (is_null ( $dirs ) || $dirs == '') {
			Zend_Loader::loadClass ( $class, array ($config ['classPath'], $config ['appPath'] . 'application/library', $config ['ZendFrameworkPath'] ) );
		} else {
	...
	public static function loadPortlet($class, $dirs = null) {
		...
		if (! empty ( $dirs )) {
			...
			Zend_Loader::_securityCheck ( $file );
			include_once $file;
		}
