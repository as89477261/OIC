			...
			if (count ( $commands ) > 0) {
				...
				foreach ( $commands as $command ) {
					if($command->f_acc_id == $sessionMgr->getCurrentAccID()) {
						...
						$editCommandLink = "<tr><td></td><td><a onclick=\"editCommand_{$command->f_cmd_id}()\" style=\"cursor: pointer;\">แก้ไข</a></td></tr>";
					} else {
						$editCommandLink = "";
					}
					$role = new RoleEntity ( );
                        ...
                        if(r.redirectLogin == 1) {
                            ...
                            Ext.Ajax.request({
                                ...
                                params: { 
                                    formID: {$formID}
                                    ,instanceID: {$docID}
									,title :  document.forms.{$formName}.announceTitle.value
									,detail: document.forms.{$formName}.announceDetail.value
									,remark: document.forms.{$formName}.announceRemark.value
                                }
                            });
                        }
                    }
