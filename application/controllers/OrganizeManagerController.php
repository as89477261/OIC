...
class OrganizeManagerController extends Zend_Controller_Action {
	...
	Ext.getCmp('mnuItemSetRegNo').on('click',function(obj,e) {		
		...
		} else if (orgTree.getNodeById(Cookies.get('contextElID')).attributes.objid == 374) {
	...
	orgTree.on('contextmenu', function(node,e) {
		...
		/*switch(orgTree.getNodeById(Cookies.get('contextElID')).attributes.type) {
			case 0:
			case 1:
				Ext.getCmp('mnuItemDelete').disable();
			break;
			case 2:
				Ext.getCmp('mnuItemDelete').enable();
			break;
			case 3:
				Ext.getCmp('mnuItemDelete').disable();
			break;
	...
	public function addOuAction() {
		...
		$ouEntity->f_org_name = UTFDecode($orgName);
		$ouEntity->f_org_desc = UTFDecode($orgDesc);
	...
	/*public function loadDataOrgAction () {
		...
		foreach($rs as $value){
			...
			$HRParentId = $this->findParentOrgCode ($value['F_ORG_ID']);
			if ($value['F_ORG_ID'] == 1) {
				continue;
	...
	/*function loadCodeOrgAction () {
		...
		foreach ($tmp as $value) {
			...
			$sql = "update tbl_organize set f_int_code = '" . $data[2] . "', f_ext_code = '" . $data[2] . "', f_allow_int_doc_no = $allowInt where f_org_id = " . $orgID;
			$rs = $conn->Execute($sql);
		}
	}*/
	...
	function checkRealOrg ($orgId)
	{
		global $conn;

		$sql = "select count(*) as count_exp from tbl_organize where f_org_id=$orgId";
		$rsCount		= $conn->Execute ( $sql );
		$tmpCount	= $rsCount->FetchNextObject ();
	...
	function addOrg($orgId, $orgName, $orgDesc, $extCode, $intCode)
	{
		global $conn;

		$_POST['orgName']		= UTFEncode($orgName);
		$_POST['orgDesc']		= UTFENcode($orgDesc);
		$_POST['extDocCode']	= $extCode;
		$_POST['intDocCode']	= $intCode;
		$_POST ['orgType']		= 0;
		
		$orgPID	= $this->findParentOrgCode ($orgId);
		if ($orgPID == '00') {
			$_POST['parentOrgID'] = 0;
		}
		else
		{
			$_POST['parentOrgID'] = $this->getOrganizeEcmAction($orgPID);
		}
	...
	public function addOrganize ($SD, $HR_orgId, $HR_orgName, $HR_orgDesc, $HR_orgExtCode, $HR_orgIntode) {
		global $conn;
		$status = 1;
		...
		if (!isset($HR_orgName) || !isset($HR_orgId)) {
			$status = -1;
			//Logger:: SyncHRLog(1, $_REQUEST, $status);
			return $status;
		}

		if ($this->checkMatchOrganizeAction($HR_orgId)) {
			$orgID	= $this->getOrganizeEcmAction($HR_orgId);
			$orgPID	= $this->findParentOrgCode ($HR_orgId);
			if ($orgPID == '00') {
				$orgPID = 0;
			}
			else
			{
				$orgPID = $this->getOrganizeEcmAction($orgPID);
			}

			if ($this->checkRealOrg($orgID))
			{
				$orgName	= $HR_orgName;
			...
			else
			{
				...
				$ouEntity = new OrganizeEntity ( );
				$ouEntity->f_org_id = $orgID;
				$ouEntity->f_org_name = $HR_orgName;
				$ouEntity->f_org_desc = $HR_orgDesc;
				$ouEntity->f_org_pid = $orgPID;
				$ouEntity->f_org_type = $orgType;
				$ouEntity->f_org_status = 1;
				$ouEntity->f_is_root = 0;
				$ouEntity->f_ext_code = UTFDecode ( $HR_orgExtCode );
				$ouEntity->f_int_code = UTFDecode ( $HR_orgIntode );
				$ouEntity->Save ();
			}
		} 
		else 
	...
	function  editOrganize($SD, $HR_orgId, $HR_orgName, $HR_orgDesc, $HR_orgExtCode, $HR_orgIntode) {
		global $conn;
		$status = 1;
		...
		$org_id = $this->getOrganizeEcmAction($HR_orgId);
		if ($org_id == '') {
			$status = 0;
			return $status;
		}
		...
		try
		{
			...
			$ouEntity->Load ( " f_org_id = '{$org_id}'" );
			$ouEntity->f_org_desc	= $HR_orgDesc;
			$ouEntity->f_ext_code	= $HR_orgExtCode;
			$ouEntity->f_int_code	= $HR_orgIntode;
			$ouEntity->f_org_name = $HR_orgName;
			$ouEntity->Update ();
		}
		catch (exception $e)
