...
class AnnounceController extends ECMController {
    ...
    public function createAction () {
		...
		//if($type == 0) {
		if($sessionMgr->getCurrentYear() == '2009') {
			if(!$sequence->isExists('command_'.$catID)) {
				$sequence->create('command_'.$catID);
			}
		} else {
			if(!$sequence->isExists('command_'.$catID.'_'.$sessionMgr->getCurrentYear())) {
				$sequence->create('command_'.$catID.'_'.$sessionMgr->getCurrentYear());
			}
		}
        //}

		if($sessionMgr->getCurrentYear() == '2009') {
			$announce->f_announce_no = $sequence->get('command_'.$catID);
		} else {
			$announce->f_announce_no = $sequence->get('command_'.$catID.'_'.$sessionMgr->getCurrentYear());
		}
    ...
    public function editAction () {
		...
		$announce = new AnnounceEntity();
		if(!$announce->Load("f_announce_id = '{$_POST['instanceID']}'")) {
			$result['success'] = 0;
		} else {
			$announce->f_remark = UTFDecode($_POST['remark']); 
			$announce->f_detail= UTFDecode($_POST['detail']);     
			$announce->f_title= str_replace("\n","",str_replace("\r","",UTFDecode($_POST['title'])));
			$announce->Update();
			$result['success'] = 1;
		}
